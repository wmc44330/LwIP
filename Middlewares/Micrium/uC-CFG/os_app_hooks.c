...
#define HOOK_DEBUG(fmt, ...) printf(fmt, ##__VA_ARGS__)
...
void  App_OS_TaskCreateHook (OS_TCB  *p_tcb)
{
    (void)&p_tcb;
    HOOK_DEBUG("\r\n====================================================\r\n");
    HOOK_DEBUG("**      Task Name :%s\r\n", p_tcb->NamePtr);
    HOOK_DEBUG("**      Task Prio :%d\r\n", p_tcb->Prio);
    HOOK_DEBUG("**      Task StkSize :%d\r\n", p_tcb->StkSize);
    HOOK_DEBUG("**      Task StkBasePtr :%#x\r\n", (unsigned int)p_tcb->StkBasePtr);
    HOOK_DEBUG("**      Task MsgQ.NbrEntriesSize :%d", p_tcb->MsgQ.NbrEntriesSize);
    HOOK_DEBUG("\r\n====================================================\r\n");
}
